...
function ENT:Fire30mmBulletAt(impactPos, bulletIndex)
    ...
    local tr = util.TraceLine({
        start  = Vector(impactPos.x, impactPos.y, self.sky + 200),
        endpos = Vector(impactPos.x, impactPos.y, -16384),
    ...
    local ed1=EffectData() ed1:SetOrigin(hitPos) ed1:SetScale(1.5) ed1:SetMagnitude(1.5) ed1:SetRadius(40) util.Effect("gred_ground_impact",ed1,true,true)
    local ed2=EffectData() ed2:SetOrigin(hitPos) ed2:SetScale(1)   ed2:SetMagnitude(1)   ed2:SetRadius(30) util.Effect("Sparks",ed2,true,true)

    net.Start("gred_net_createimpact")
        net.WriteVector(hitPos) net.WriteAngle(Angle(0,0,0))
        net.WriteUInt(0, 5) net.WriteUInt(GAU_CAL_ID, 4)
    net.Broadcast()
    ...
    if tr.Hit and IsValid(tr.Entity) and tr.Entity ~= self then
        ...
        if ent:IsPlayer() or ent:IsNPC() or ent:GetClass() == "nextbot" then
            ...
            dmginfo:SetAttacker(self) dmginfo:SetDamage(self.GAU_BulletDamage)
            dmginfo:SetDamagePosition(hitPos) dmginfo:SetDamageType(DMG_BULLET)
            ent:TakeDamageInfo(dmginfo)
        end
    end

    if bulletIndex % self.GAU_HEI_Interval == 0 and HasGred() then
        local shell = gred.CreateShell(hitPos+Vector(0,0,30), Angle(90,0,0), self, {self}, 20, "HE", 80, 0.1, nil, 60, nil, 0.005)
